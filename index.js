    ...
    ws.on('message', (data) => {
        ...
        for (const txHash in message) {
            ...
            for (const userAction of userActions) {
                ...
                const msg =
                `🚨 ${process.env.PROTOCOL.toUpperCase()} — ${action.toUpperCase()} ${formatBigNumber(Number(tokens[0].ui_amount))} <a href="${EXPLORER_URL}/token/${tokens[0].address}">${tokens[0].symbol}</a> ($${formatBigNumber(Number(valueUsd))})\n`
